...
> **Mastra runtime binding:** `pipeline.md` is authoritative for exact tool names, state persistence,
> approvals, filesystem paths, and generation mechanics. This file is authoritative for filmmaking
> judgment, routing, continuity, and prompt quality. Platform/provider facts are volatile; the Source
> Gate and current runtime evidence override cached examples in this skill.
...
1. Intake: identify the user's goal, production phase, target engine, duration, aspect ratio, references, and audio needs.
   - **Default Route**: For YouTube videos, podcasts, talking head cuts, B-roll graphics, kinetic typography, charts, or video editing, ALWAYS route to the **local free YouTube Video Engine (Remotion TSX + FFmpeg)**.
   - **Seedance Route**: ONLY route to Seedance generative video if the user explicitly specified "Seedance" in their request.
...
10. Prompt build: route to `[skill:seedance-interview]`, `[skill:seedance-prompt]`, `[skill:seedance-prompt-short]`, `[skill:seedance-sequence]`, `[skill:seedance-continuation]`, or a domain skill for camera, motion, audio, characters, VFX, style, recipes, or pipeline.
11. Quality pass: run anti-slop, check one visible beat, one primary camera move, physical light, sound intent, continuity anchors, constraints, delivery caveats, and source-date caveats.
12. Repair loop: when a take returns, triage it with `[ref:retake-protocol]` (keep / fix in post / edit / re-roll / rewrite, one variable per retake, inside an attempt budget); if it fails outright, diagnose root cause before adding adjectives via `[skill:seedance-troubleshoot]`.
