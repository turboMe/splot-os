---
name: promo-writer
description: "This skill should be used when writing platform-specific promotional copy for tracks, albums, release announcements, artist posts, captions, descriptions, or campaign messaging."
license: CC0-1.0
user-invocable: true
tags: [promotion, copywriting, release, social]
metadata:
  version: "1.0.0"
  updated: "2026-06-23"
  parent: "musician"
  source_repo: "bitwize-music-studio/claude-ai-music-skills"
  source_skill: "promo-writer"
...
This is a Mastra musician-domain port of the CC0 `promo-writer` skill from `bitwize-music-studio/claude-ai-music-skills`. Use the craft guidance below, but obey the local runtime contract first.
...
- Load `copy-formulas.md` with `music_load_reference` using `kind:"skill"`, `name:"promo-writer/copy-formulas"`.
...
Use local music tooling `find_album` with the album name from `$ARGUMENTS`. If no album specified, check `get_session` for last album context.
...
3. **Streaming lyrics**: from album data sections — pull quotable hooks from streaming lyrics (NOT active music surface lyrics, which contain phonetic spellings)
4. **User preferences**: project/user promotion preferences — tone, platform priorities, messaging themes, hashtag preferences, AI positioning

**Critical**: Use **streaming lyrics** for quotable hooks. active music surface lyrics contain phonetic spellings (`bit-wize`, `Luh-rock-uh`) that must never appear in public-facing copy.
...
Next steps:
  1. Review and polish: [skill:promo-reviewer] <album-name>
  2. Replace [Streaming Link] placeholders with actual URLs when available
  3. When ready to release: [skill:release-director] <album-name>
```
...
### distribution lyrics Only
Pull quotable hooks from **streaming lyrics** sections. Never use active music surface lyrics — they contain phonetic spellings meant for the AI, not human readers.
...
2. **Streaming lyrics only** — never active music surface phonetic lyrics in public copy
