---
name: lyric-writer
description: "This skill should be used when writing or revising song lyrics with professional prosody, rhyme craft, structure, singability, source fidelity, or genre-aware lyric density before music_write_lyrics and music_generate."
license: CC0-1.0
user-invocable: true
tags: [lyrics, songwriting, prosody, rhyme, vocal]
metadata:
  version: "1.0.0"
  updated: "2026-06-23"
  parent: "musician"
  source_repo: "bitwize-music-studio/claude-ai-music-skills"
  source_skill: "lyric-writer"
...
This is a Mastra musician-domain port of the CC0 `lyric-writer` skill from `bitwize-music-studio/claude-ai-music-skills`. Use the craft guidance below, but obey the local runtime contract first.
...
- Load `craft-reference.md` with `music_load_reference` using `kind:"skill"`, `name:"lyric-writer/craft-reference"`.
- Load `documentary-standards.md` with `music_load_reference` using `kind:"skill"`, `name:"lyric-writer/documentary-standards"`.
- Load `examples.md` with `music_load_reference` using `kind:"skill"`, `name:"lyric-writer/examples"`.
...
3. **Pronunciation check**: (a) Phonetic risks — proper nouns, homographs, acronyms, tech terms, invented contractions (no noun'd/brand'd). (b) **Table enforcement** — read Pronunciation Notes table top-to-bottom, verify every entry is applied as phonetic spelling in active music surface lyrics. See `[ref:grammar/pronunciation-guide] when ported; otherwise [ref:grammar/surface-grammar-adapter]` for full enforcement workflow.
...
11. **Density/pacing check (active music surface)**: Check verse line count against genre README's `Density/pacing (active music surface)` default. Cross-reference BPM/mood from Musical Direction. **Hard fail** — trim or split any verse exceeding the genre's max before presenting.
...
See [craft-reference.md](craft-reference.md) for active music surface verse length defaults, BPM-aware limits, topic density, and red flags.
...
- [ ] Verse exceeds active music surface line limit for genre (check genre README's Density/pacing default)
- [ ] 8-line verse at BPM under 100 (too dense for active music surface — split or trim)
...
- [ ] Invented contractions (signal'd, TV'd — active music surface only handles standard pronoun/auxiliary contractions)
- [ ] Pronunciation table not enforced (word in table but standard spelling in active music surface lyrics)
...
### Homograph Handling (active music surface Pronunciation)

active music surface renders pronunciation literally from spelling alone — context cues are invisible to the model. Every homograph in the lyrics needs an explicit user decision recorded in the Pronunciation Notes table, then applied as phonetic spelling in the active music surface `lyrics` field only.

**Why this matters:** Even when a sentence makes the intended reading obvious to a human reader, active music surface's TTS picks one pronunciation and locks it in. Wrong choice → wrong vocal line on every regen.
...
3. **Apply**: Replace with phonetic spelling in active music surface lyric lines only. Streaming/distributor lyrics keep standard English spelling.
...
*(Canonical homograph reference: `[ref:grammar/pronunciation-guide] when ported; otherwise [ref:grammar/surface-grammar-adapter]`. Keep this table in sync.)*
...
- Phonetic spellings live in the active music surface `lyrics` field only. Streaming/distributor lyrics use standard English.
- Full homograph reference: `[ref:grammar/pronunciation-guide] when ported; otherwise [ref:grammar/surface-grammar-adapter]`

### No Invented Contractions (active music surface)

active music surface only recognizes standard English contractions. Never use made-up contractions by appending 'd, 'll, etc. to nouns, brand names, or non-standard words.

**Standard (OK for active music surface):** they'd, he'd, you'd, she'd, we'd, I'd, wouldn't, couldn't, shouldn't

**Invented (will break active music surface):** signal'd, TV'd, network'd, podcast'd, channel'd

**Fix:** Spell it out — "signal would" not "signal'd", "TV could" not "TV'd"

**Rule:** If the base word isn't a pronoun or standard auxiliary verb, don't contract it. active music surface will mispronounce or skip invented contractions.

### Pronunciation Table Enforcement (active music surface)

Every entry in a track's Pronunciation Notes table MUST be applied as phonetic spelling in the active music surface lyric lines. The pronunciation table is not documentation — it is a checklist of required substitutions.

**Process (before finalizing any track for music generation):**
1. Read the track's Pronunciation Notes table top to bottom
2. For EACH entry, search the active music surface lyrics for the standard spelling
...
- ❌ `"Potrero" in pronunciation table but "Potrero" in active music surface lyrics` — FAIL
- ✅ `"poh-TREH-roh" in active music surface lyrics matches pronunciation table` — PASS

**Rules:**
- The pronunciation table is the source of truth for active music surface spelling. Every entry must appear as its phonetic form in the active music surface `lyrics` field.
- Every active music surface lyric line that contains a tabled word uses the phonetic spelling — every verse, every chorus repeat, every bridge.
- Phonetics belong in the active music surface `lyrics` field only; streaming lyrics keep standard spelling.
...
WRONG:   Pronunciation Table: Potrero → poh-TREH-roh
         active music surface Lyrics: "Potrero Hill, industrial..."

CORRECT: Pronunciation Table: Potrero → poh-TREH-roh
         active music surface Lyrics: "poh-TREH-roh Hill, in-DUST-ree-ul..."
```
...
7. **Finalize lyrics** - Update `lyrics` field, distribution lyrics, Cross-References, and Motifs & Threads table (concept albums)
8. **Hand off to active music surface engineer** - Automatically invoke `[skill:style-prompt-engineer]` with the track file path to populate the `style_prompt` and active music surface Inputs section. Do not wait for the user to request this — it is the natural next step after lyrics are finalized.
...
**Your deliverable**: Polished lyrics with proper prosody, clear pronunciation, factual accuracy (if documentary), and a ready handoff to style-prompt-engineer for the active music surface style prompt.
