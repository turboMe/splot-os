...
description: "This skill should be used for mastering knowledge, loudness targets, streaming delivery specs, tonal balance, and platform preparation. Execution is deferred; use as guidance only until DSP tooling exists."
license: CC0-1.0
user-invocable: true
tags: [mastering, loudness, streaming, delivery]
metadata:
  version: "1.0.0"
  updated: "2026-06-23"
  parent: "musician"
  source_repo: "bitwize-music-studio/claude-ai-music-skills"
  source_skill: "mastering-engineer"
  execution: "deferred"
...
This is a Mastra musician-domain port of the CC0 `mastering-engineer` skill from `bitwize-music-studio/claude-ai-music-skills`. Use the craft guidance below, but obey the local runtime contract first.
...
- Load `genre-presets.md` with `music_load_reference` using `kind:"skill"`, `name:"mastering-engineer/genre-presets"`.
...
### Loading Override
1. Consult user/project preferences for `mastering-presets.yaml` if available in memory, project notes, or future override storage.
...
Before mastering, resolve audio path via local music tooling:
...
**Do not** use placeholder paths or assume audio locations — always resolve via local music tooling.
...
3. If no WAV files found, report: "No WAV files in [path]. Download tracks from active music surface as WAV (highest quality) first."
4. If folder contains only MP3s, warn: "MP3 files found but mastering requires WAV. Re-download from active music surface as WAV."
...
Use the `master_album` local music tooling tool to run **Steps 2–7 in a single call**:
...
All mastering operations are available as local music tooling tools. **Use these instead of running Python scripts via bash.**
...
### After active music surface Generation
active music surface outputs vary in loudness - some at -8 LUFS, some at -18 LUFS.
...
**Why it matters:** Bash hits system Python which lacks dependencies. local music tooling tools run inside the venv automatically.
...
1. **Load override first** - Consult user/project preferences for `mastering-presets.yaml` if available in memory, project notes, or future override storage.
