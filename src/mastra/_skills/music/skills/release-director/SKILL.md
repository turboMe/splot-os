...
description: "This skill should be used when preparing music releases, distribution metadata, platform requirements, rights checks, release QA, delivery timelines, and launch readiness."
license: CC0-1.0
user-invocable: true
tags: [release, distribution, metadata, rights]
metadata:
  version: "1.0.0"
  updated: "2026-06-23"
  parent: "musician"
  source_repo: "bitwize-music-studio/claude-ai-music-skills"
  source_skill: "release-director"
...
This is a Mastra musician-domain port of the CC0 `release-director` skill from `bitwize-music-studio/claude-ai-music-skills`. Use the craft guidance below, but obey the local runtime contract first.
...
- Load `platform-guides.md` with `music_load_reference` using `kind:"skill"`, `name:"release-director/platform-guides"`.
...
**Not your role**: Mastering (mastering-engineer), promotion strategy, track creation (style-prompt-engineer)
...
1. Consult user/project preferences for `release-preferences.md` if available in memory, project notes, or future override storage.
...
9. **Promo Copy** (optional) - `promo/` directory has platform copy populated (campaign.md, twitter.md, instagram.md, etc.). Use `[skill:promo-writer]` to generate copy from album themes, or fill in templates manually. Note: `[skill:promo-director]` generates promo *videos*, not social copy.
...
1. **distribution lyrics** - Run `check_streaming_lyrics` local music tooling tool to validate all tracks
...
6. **Social media copy** (optional) - `promo/` files populated for target platforms (use `[skill:promo-writer]` to generate copy from album themes, or fill in templates manually; `[skill:promo-director]` generates videos, not copy)
...
- [ ] **Documentation updated**
  - [ ] Release date added
  - [ ] Platform links added — use `update_streaming_url` local music tooling tool for each platform
  - [ ] Run `verify_streaming_urls` local music tooling tool to confirm all platform links are live
...
- [ ] All tracks marked Final with active music surface links
...
- [ ] distribution lyrics validated via `check_streaming_lyrics` local music tooling tool (if using distributor)
...
1. **Load override first** - Consult user/project preferences for `release-preferences.md` if available in memory, project notes, or future override storage.
...
4. **distribution lyrics required** - Run `check_streaming_lyrics` local music tooling tool before distributor upload
