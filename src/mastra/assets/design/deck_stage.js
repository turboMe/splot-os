 ...
 * - Fixed-size canvas (default 1920x1080) + auto-scale + letterbox
 ...
 * - Speaker notes postMessage (supports external rendering)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render Shadow DOM first (independent of child nodes, unaffected by parser timing)
      this._render();

      // Defensive: If the script is placed in <head> (instead of after </deck-stage>),
      // the parser might not have finished processing child <section> elements yet, and querySelectorAll would return empty.
      // Delay to the next event loop to ensure all child nodes are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document is still parsing, wait for DOMContentLoaded to handle all sections at once
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document has finished parsing (script at bottom of body or defer), collect in the next frame
