...
DEFAULT_MODEL = "gpt-image-2"          # Default: OpenAI gpt-image-2; fallback: dall-e-3 or gemini-2.5-flash-image
...
def main():
    ...
    gemini_key = env.get("GEMINI_API_KEY", env.get("GOOGLE_GENERATIVE_AI_API_KEY", "")).strip()

    is_openai_model = "gpt" in model.lower() or "dall-e" in model.lower() or "dalle" in model.lower()
    img_bytes = None
    used_model = model

    # Try OpenAI first if requested or if openai key is available and model is openai
    if is_openai_model:
        if not openai_key:
            print("  ! OPENAI_API_KEY not set in .env. Falling back to Google Gemini image model.")
            ...
            try:
                ...
                # Format prompt with text hook instructions and aspect ratio
                prompt_full = prompt
                if refs:
                    prompt_full += f"\n(Presenter style reference attached: realistic, tech-aesthetic, cinematic lighting)"

                # Map to standard OpenAI image size
                img_size = "1792x1024" if aspect == "16:9" else "1024x1024"
                openai_target_model = "dall-e-3" if "dall-e" in model.lower() else "dall-e-3" # or gpt-image-2 if available

                print(f"Calling OpenAI image generation with {openai_target_model} (size={img_size})...")
                ...
                if b64_data:
                    ...
                    used_model = f"openai/{openai_target_model}"
            except Exception as e:
                print(f"  ! OpenAI generation error: {e}. Falling back to Google Imagen/Gemini...")
                img_bytes = None

    # Fallback or direct Google Gemini/Imagen generation
    if not img_bytes:
        if not gemini_key:
            sys.exit("Neither OPENAI_API_KEY nor GEMINI_API_KEY/GOOGLE_GENERATIVE_AI_API_KEY is available in .env.")
        ...
        google_model = "gemini-2.5-flash-image" if model == "gpt-image-2" else model
        client = genai.Client(api_key=gemini_key)
        try:
            resp = client.models.generate_content(
                model=google_model, contents=contents,
                config=types.GenerateContentConfig(**cfg_kwargs))
            used_model = f"google/{google_model}"
        except Exception as e:
            sys.exit(f"Google Gemini Image API error: {type(e).__name__}: {e}")
    ...
    if not img_bytes:
        sys.exit("No image returned by image model.")
    ...
    with open(sidecar, "w", encoding="utf-8") as f:
        json.dump({
            "prompt": prompt, "model": used_model, "aspect_ratio": aspect, "image_size": size,
